...
namespace sikayetvar.Controllers
{
    public class EditorController : Controller
    {
        ...
        public IActionResult Update(Post model)
        {
            ...
            {
                _connection.Execute("UPDATE Posts SET Title  = @Title, IsComplete = @IsComplete WHERE Id = @Id", model);
            }
        ...
        {
            _connection.Execute("UPDATE Posts SET IsComplete = 1 WHERE Id = @id", new { id });
            return RedirectToAction("Index");
        }

        public IActionResult MarkInComplete(int id)
        {
            _connection.Execute("UPDATE Posts SET IsComplete = 0 WHERE Id = @id", new { id });
            return RedirectToAction("Index");
        }
    }
}